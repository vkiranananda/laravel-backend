...
class UserAccess
{
    public static function getRole()
    {
        ...
        if (!$role) {
            ...
            if (!$data)
                abort(403, 'UserAccess::getRole role not found');
    ...
    public static function checkAccess($access, $modKey, $userId = false)
    {
        ...
        $checkOwner = function ($key) use ($userId, $perm) {
            ...
            if ($perm[$key] == 'owner' && (!$userId || $userId == '' || $userId == Auth::user()->id))
                return true;
            // По умолчанию запрет
            return false;
        };
     ...
     * @return bool - Вернет true или false.
     */
    public static function checkCustomAccess($accessName, $modKey, $userId = false)
    {
        $role = UserAccess::getRole();

        if (!isset($role[$modKey][$accessName]))
            return false;

        return $role[$modKey][$accessName];
    }
