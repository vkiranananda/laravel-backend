...
export default {
    ...
    mutations: {
        initData(state, { fields, config }) {
            ...
            state.uploadFiles = config['clone-files']
                ? config['clone-files']
                : [];
            ...
            for (var name in state.tabs) {
                ...
                for (let fieldName of tabFields)
                    if (state.fields[fieldName] != undefined) {
        ...
        addRepeatedBlock(state, field) {
            field.value.push({
                fields: cloneDeep(field['fields']),
                key: field['unique-index'],
            });
            field['unique-index']++;
        },
    ...
    actions: {
        ...
        addRepeatedBlock({ commit, state }, field) {
            ...
            setVShowData(
                commit,
                field.value[field.value.length - 1].fields,
                true,
            );
        },
        //Устанавливаем value
        setFieldProp({ commit, state }, data) {
            ...
            if (data.property == 'value') {
                ...
                if (field.type == 'select' || field.type == 'radio') {
                    if (data.fieldsType == 'tab')
                        setVShowDataRoot(commit, state);
                ...
                if (!field.autosave && field.changed !== false) {
        ...
        initData({ commit, dispatch, state }, data) {
            ...
            if (state.tabActive != '') {
                ...
                if (
                    state.tabs[state.tabActive] != undefined &&
                    state.tabs[state.tabActive]['v-show'] !== false
                ) {
        ...
        if (all === true) {
            if (field.type == 'repeated') {
                for (let repDataBlock of field.value)
                    setVShowData(commit, repDataBlock.fields, all);
            }
...
function setVShowDataRoot(commit, state, all) {
    ...
    for (var tabName in state.tabs) {
        ...
        if (state.tabs[tabName]['show'] != undefined) {
            commit('setTabVShow', {
                name: tabName,
                value: vShowCheck(state.tabs[tabName]['show'], state.fields),
            });
        }
    }
