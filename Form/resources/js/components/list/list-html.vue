<template>
  <div class="list-html">
    <h3 class="title mb-4" v-html="myData.config.title"></h3>
    ...
    <div class="mb-3">
      <button
        v-for="(el, key) in myData.config.menu"
        class="btn me-3"
        :class="
          el['btn-type'] ? 'btn-' + el['btn-type'] : 'btn-outline-secondary'
        "
        role="button"
        v-on:click.stop.prevent="menuActionClick(el)"
      >
        <v-icon v-if="el.icon" :name="el.icon" :class="el['label'] && el['label'] != '' ? 'me-1' : ''" />
    ...
    <the-list-components
      v-if="myData.components['after-buttons']"
      components="after-buttons"
      :data="myData"
    ></the-list-components>
    <the-search
      v-if="myData.search != undefined"
      :fields="myData.search"
      @v-change="searchChange"
    ></the-search>
    <the-list-components
      v-if="myData.components['after-search']"
      components="after-search"
      :data="myData"
    ></the-list-components>
    <the-breadcrumbs
      v-if="myData.breadcrumbs"
      :data="myData.breadcrumbs"
    ></the-breadcrumbs>
    <the-list
      :fields="myData.fields"
      :items="myData.items"
      :itemMenu="myData.itemMenu"
      @v-change="listChange"
    ></the-list>
...
export default {
  ...
  methods: {
    ...
    pageReload() {
      ...
      axios
        .get(window.location.href, { params: { _ajax: Math.random() } })
        .then((response) => {
          this.myData = response.data;
        })
        .catch((error) => {
