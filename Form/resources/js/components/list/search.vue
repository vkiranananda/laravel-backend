    ...
    <form action="" v-on:submit.prevent="search()">
        <div class="card mb-3">
            <div class="card-body pb-2" v-if="fieldsNew.fields.length > 0">
                ...
                <div class="row">
                    <div
                        v-for="field in fieldsNew.fields"
                        class="form-group" :class="field['col-classes'] ? field['col-classes'] : 'col-4'"
                    >
                        <label v-if="field.label">{{ field.label }}</label>
                        <print-field
                            :field="field"
                            @v-change="onChange($event, field)"
                        ></print-field>
                    </div>
        ...
        <div class="d-flex flex-wrap align-items-end gap-2 mx-2 mb-3" v-if="fieldsNew.filters.length > 0">
          <span class="me-2 align-self-center">Фильтры:</span>
            <div
                v-for="field in fieldsNew.filters"
                class="form-group"
            >
                <print-field
                    :field="field"
