    ...
    <div class="field-block d-flex align-items-center">
        <div class="field">
            ...
            <component
                ...
                :fields="fields"
                :error="error"
                v-on:v-change="$emit('v-change', $event)"
            ></component>
        </div>
        <v-icon
            v-if="field.protected === true && blocked"
            name="key"
            class="back"
            @click="unblock"
        />
...
import FormFieldFiles from '../../../../../MediaFile/resources/js/components/files-field.vue';
import FormFieldImage from '../../../../../MediaFile/resources/js/components/image-field.vue';
...
export default {
    ...
    computed: {
        component() {
            if (
                ['text', 'email', 'password', 'tel'].indexOf(this.field.type) !=
                -1
            )
                return 'input';
            if (['gallery', 'files'].indexOf(this.field.type) != -1)
                return 'files';
    ...
    methods: {
        unblock: function () {
            let msg = this.field['protected-message']
                ? this.field['protected-message']
