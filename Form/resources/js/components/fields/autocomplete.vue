<template>
  <div class="autocomplete">
    ...
    <div
      v-show="showResults"
      class="results text-secondary rounded-bottom"
      ref="results"
    >
      <div
        v-for="(el, key) in searchData"
        :key="key"
        class="item p-2"
        :class="focus.value == el.value ? 'bg-light' : ''"
...
export default {
  ...
  methods: {
    ...
    clickOutside: function (event) {
      if (
        !(
          this.$refs.results == event.target ||
          this.$refs.results.contains(event.target) ||
    ...
    select: function () {
      if (this.showResults) {
                  // console.log(this.focus.value, this.field.value);
    ...
    searchItem: function (search) {
      for (let i in this.searchData)
        if (this.searchData[i].value == search) return parseInt(i);
      return false;
    },
    // Клавиша вниз
    down: function (event) {
